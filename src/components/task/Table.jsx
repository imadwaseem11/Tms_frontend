...
const Table = ({ tasks }) => {
  ...
  const deleteClicks = (id) => {
    if (typeof id !== 'string') {
      console.error('Expected a string ID but received:', id);
      return;
    }
    setSelected(id);
    setOpenEdit(false);  // Ensure edit dialog is closed
    setOpenDialog(true);  // Open the delete dialog
  };
  ...
  const editTaskHandler = (el) => {
    ...
    setOpenDialog(false);  // Ensure delete dialog is closed
  };

  const deleteHandler = async () => {
    try {
      console.log('Selected ID:', selected);  // Debug the ID
